...
import {
  ...
  ScatterChart, Scatter, ZAxis, Cell, Rectangle
} from 'recharts';
...
const ChartCard: React.FC<{ title: string; subtitle?: string; action?: React.ReactNode; children: React.ReactNode }> = ({ title, subtitle, action, children }) => (
      ...
      <div className="bg-gray-800 border border-gray-600 p-3 rounded-lg shadow-2xl text-xs z-50 relative min-w-[200px]">
        <div className="mb-2 pb-2 border-b border-gray-700">
          ...
          <p className="text-indigo-400 font-medium mt-1">
            {mainName}: {typeof mainValue === 'number' && mainName.includes('Revenue') ? `$${mainValue}M` : mainValue}
          </p>
        </div>
        
        {topMovies.length > 0 && (
          <div>
            <p className="text-gray-400 text-[10px] uppercase tracking-wider mb-1">Notable Movies</p>
            <ul className="space-y-1">
              {topMovies.map((m) => (
                <li key={m.id} className="flex justify-between items-center text-gray-300">
                  <span className="truncate max-w-[140px] font-medium" title={m.title}>{m.title}</span>
...
export const RatingHistogram: React.FC<ChartProps> = ({ data, filters }) => {
  const chartData = useMemo(() => {
    ...
    // 2. Distribute movies
    data.forEach(m => {
      let rating = filters.ratingSource === 'imdb' ? m.imdbRating : m.rottenTomatoesRating / 10;
  ...
  return (
    <ChartCard title="Rating Distribution" subtitle={`Based on ${filters.ratingSource === 'imdb' ? 'IMDb' : 'Rotten Tomatoes'} scores`}>
  ...
  const chartData = useMemo(() => {
    ...
    return Object.entries(genreStats)
      .map(([genre, stats]) => ({
        ...
        topMovies: stats.movies.sort((a, b) => b.revenue - a.revenue).slice(0, 5) // Highest earners
      }))
      .sort((a, b) => b.avgRevenue - a.avgRevenue);
  }, [data]);

  return (
    <ChartCard title="Avg. Revenue by Genre" subtitle="Average box office (Millions)">
      <ResponsiveContainer width="100%" height="100%">
  ...
  const chartData = useMemo(() => {
    ...
    return Object.entries(yearGroups)
      .map(([year, movies]) => ({ 
        year: parseInt(year), 
        count: movies.length,
        topMovies: movies.sort((a, b) => b.revenue - a.revenue).slice(0, 5) // Top by revenue
      }))
...
export const BudgetRatingScatter: React.FC<ChartProps> = ({ data, filters }) => {
  const chartData = useMemo(() => {
    return data.map(m => ({
      x: m.budget,
      y: filters.ratingSource === 'imdb' ? m.imdbRating : m.rottenTomatoesRating,
      title: m.title,
      genre: m.genre
    }));
  }, [data, filters.ratingSource]);

  const yDomain = filters.ratingSource === 'imdb' ? [0, 10] : [0, 100];
  const yLabel = filters.ratingSource === 'imdb' ? 'IMDb' : 'RT Score';
    ...
    <ChartCard title="Budget vs. Rating Correlation" subtitle="Does money buy quality?">
      <ResponsiveContainer width="100%" height="100%">
        <ScatterChart margin={{ top: 20, right: 20, bottom: 20, left: 0 }}>
          ...
          <XAxis type="number" dataKey="x" name="Budget" unit="M" stroke="#9CA3AF" tick={{ fontSize: 12 }}>
            {/* <Label value="Budget ($M)" offset={-10} position="insideBottom" fill="#9CA3AF" /> */}
          </XAxis>
          <YAxis type="number" dataKey="y" name="Rating" domain={yDomain} stroke="#9CA3AF" tick={{ fontSize: 12 }}>
              ...
              if (active && payload && payload.length) {
                ...
                return (
                  <div className="bg-gray-800 border border-gray-600 p-2 rounded shadow-lg text-xs z-50 relative">
                    ...
                    <p className="text-gray-300">Genre: {data.genre}</p>
                    <p className="text-indigo-400">Budget: ${data.x}M</p>
                    <p className="text-green-400">
                      {filters.ratingSource === 'imdb' ? 'IMDb Rating' : 'RT Score'}: {data.y}
                    </p>
                  </div>
                );
              }
...
export const GenreSpreadPlot: React.FC<ChartProps> = ({ data, filters }) => {
   const chartData = useMemo(() => {
    return data.map(m => ({
      genre: m.genre,
      rating: filters.ratingSource === 'imdb' ? m.imdbRating : m.rottenTomatoesRating,
      title: m.title
    }));
  }, [data, filters.ratingSource]);
  ...
  return (
    <ChartCard title="Rating Spread by Genre" subtitle="Distribution of scores across categories">
      <ResponsiveContainer width="100%" height="100%">
        <ScatterChart margin={{ top: 20, right: 20, bottom: 20, left: 0 }}>
          <CartesianGrid strokeDasharray="3 3" stroke="#374151" />
          <XAxis dataKey="genre" type="category" stroke="#9CA3AF" tick={{ fontSize: 12 }} allowDuplicatedCategory={false} />
          <YAxis dataKey="rating" type="number" domain={yDomain} stroke="#9CA3AF" tick={{ fontSize: 12 }} name="Rating" />
          <Tooltip 
            cursor={{ strokeDasharray: '3 3' }}
            content={({ active, payload }) => {
              if (active && payload && payload.length) {
                ...
                return (
                  <div className="bg-gray-800 border border-gray-600 p-2 rounded shadow-lg text-xs z-50 relative">
                    <p className="font-bold text-white">{data.title}</p>
                    <p className="text-yellow-400">Rating: {data.rating}</p>
                  </div>
                );
              }
              return null;
...
export const RevenueByGenreScatter: React.FC<ChartProps> = ({ data }) => {
  ...
  return (
    <ChartCard title="Revenue by Genre" subtitle="Box office performance distribution">
      <ResponsiveContainer width="100%" height="100%">
        <ScatterChart margin={{ top: 20, right: 20, bottom: 20, left: 0 }}>
          <CartesianGrid strokeDasharray="3 3" stroke="#374151" />
          <XAxis dataKey="genre" type="category" stroke="#9CA3AF" tick={{ fontSize: 12 }} allowDuplicatedCategory={false} />
          <YAxis dataKey="revenue" type="number" name="Revenue" unit="M" stroke="#9CA3AF" tick={{ fontSize: 12 }} />
          <Tooltip 
            cursor={{ strokeDasharray: '3 3' }}
            content={({ active, payload }) => {
              if (active && payload && payload.length) {
                ...
                return (
                  <div className="bg-gray-800 border border-gray-600 p-3 rounded-lg shadow-xl text-xs z-50 relative min-w-[150px]">
                    <p className="font-bold text-white text-sm mb-2 pb-1 border-b border-gray-700">{data.title}</p>
                    <div className="space-y-1">
                       <div className="flex justify-between items-center">
                        <span className="text-gray-400">Budget:</span>
                        <span className="text-pink-400 font-semibold">${data.budget}M</span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="text-gray-400">Revenue:</span>
                        <span className="text-emerald-400 font-semibold">${data.revenue}M</span>
                      </div>
                      <div className="flex justify-between items-center">
