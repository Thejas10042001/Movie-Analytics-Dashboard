...
async function fetchMoviesForPeriod(startYear: number, endYear: number): Promise<Movie[]> {
  ...
  const prompt = `Generate a dataset of ${count} REAL, VERIFIABLE movies released between ${startYear} and ${endYear}.
      ...
      1. REAL TITLES ONLY: Every movie title must correspond to an actual film released in theaters or on major global streaming platforms.
         - ABOLUTELY NO FAKE SEQUELS or placeholder names.
        ...
        responseSchema: {
          ...
          items: {
            ...
            properties: {
              ...
              language: { type: Type.STRING, enum: ['English', 'Hindi', 'Tamil', 'Telugu', 'Malayalam', 'Kannada'] },
              genre: { type: Type.STRING, enum: ['Action', 'Drama', 'Comedy', 'Sci-Fi', 'Horror', 'Romance', 'Thriller', 'Adventure'] },
              year: { type: Type.INTEGER },
              imdbRating: { type: Type.NUMBER, description: "Rating from 0 to 10" },
              rottenTomatoesRating: { type: Type.INTEGER, description: "Rating from 0 to 100" },
              budget: { type: Type.NUMBER, description: "Budget in millions of USD (approximate)" },
              revenue: { type: Type.NUMBER, description: "Revenue in millions of USD (approximate)" }
            },
            required: ['title', 'genre', 'language', 'year', 'imdbRating', 'rottenTomatoesRating', 'budget', 'revenue']
          }
        }
      }
    });
...
export const generateMovieDataset = async (): Promise<Movie[]> => {
  // Split the request into 5 parallel batches to maximize data volume and avoid token limits
  // Total Target: ~900 films
  const periods = [
    [1980, 1989],
    [1990, 1999],
    [2000, 2009],
  ...
  try {
    ...
    // Re-index IDs
    const finalMovies = Array.from(uniqueMap.values()).map((movie, index) => ({
      ...movie,
      id: (index + 1).toString()
    }));
